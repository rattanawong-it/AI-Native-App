...
### ทดสอบผ่านหน้าจอด้วยบัญชีจริง — ผ่าน

กดจริงบน `localhost:3000` ด้วยบัญชีระดับ admin ต่อกับ Neon จริง · **ไม่มี error ใน log เลย**
ทุก request ตอบ 200/201

**ครุภัณฑ์**

| ทดสอบ | ผล |
|---|---|
| เพิ่มครุภัณฑ์โดยเว้นรหัสว่าง | ได้ `AS-256909-0001` · เพิ่มใบที่สองได้ `AS-256909-0002` (รันนิ่งเดินถูก) |
| วันที่ / ราคา รูปแบบไทย | "15 พฤศจิกายน 2569" · "32,500.00 บาท" (NFR4) |
| ป้ายเตือนประกัน | "ประกันเหลือ 74 วัน" — ตรงกับ 2 ก.ย. → 15 พ.ย. |
| จ่ายให้ผู้ครอบครอง | สถานะ ในคลัง → ใช้งาน · ผู้ครอบครองขึ้นถูกคน · ประวัติบันทึก "ถึง <ชื่อ>" |
| ส่งซ่อม | สถานะ → ส่งซ่อม · **ล้างผู้ครอบครองอัตโนมัติ** · ประวัติบันทึก "จาก <ชื่อ>" |
| ช่องผู้รับตามการกระทำ | เลือก "ส่งซ่อม" แล้วช่องผู้รับถูกปิดและ `*` หายไปเอง |
| validation ฝั่งหน้าจอ | กดบันทึก "จ่ายให้ผู้ครอบครอง" โดยไม่เลือกคน → "กรุณาระบุผู้รับครุภัณฑ์" |
| QR Code | เรนเดอร์บนหน้ารายละเอียด |
| ลบครุภัณฑ์ | กล่องยืนยันขึ้น → ลบแล้วเด้งกลับหน้ารายการ |

**คำขออนุมัติ**

| ทดสอบ | ผล |
|---|---|
| รายชื่อผู้อนุมัติ | กรองเหลือเฉพาะ `manager`/`admin` · คนที่เลือกแล้วหายจากรายการ (กันซ้ำ) |
| จัดลำดับขั้น | เลข 1/2 ขึ้นถูก · ปุ่มเลื่อนขึ้นของขั้นแรกและเลื่อนลงของขั้นสุดท้ายถูก disable |
| ยื่นคำขอ | ได้ `RQ-256909-0001` · สถานะ "รออนุมัติ" · ขั้น 1 ถูกไฮไลต์เป็นขั้นปัจจุบัน |
| การ์ดพิจารณา (F7.12) | ขึ้นเฉพาะกับผู้อนุมัติของขั้นที่รออยู่ |
| **อนุมัติขั้น 1** | ขั้น 1 → "อนุมัติ" พร้อมเวลาและความเห็น · **ขั้น 2 กลายเป็นขั้นปัจจุบัน** · ใบยังเป็น "รออนุมัติ" ไม่กระโดดเป็นอนุมัติแล้ว · **การ์ดพิจารณาหายไปเพราะไม่ใช่คิวเราอีกต่อไป** |
| Timeline (F7.14) | 3 เหตุการณ์: สร้างคำขอ → ยื่นคำขอ → อนุมัติขั้นที่ 1 |

**ข้อบกพร่องที่เจอและแก้แล้ว:** ครุภัณฑ์ที่ขึ้นทะเบียนโดยยังไม่มีผู้ครอบครอง ถูกบันทึกประวัติ
เป็น action `return` ทำให้แถวแรกอ่านว่า "คืนคลัง" ทั้งที่ไม่เคยมีใครถือ → เพิ่ม action `register`
(ป้าย "ขึ้นทะเบียน") แยกออกมา และกันไม่ให้เลือกเองจากเมนู · ทดสอบซ้ำแล้วแสดงถูก (commit `2b53a86`)

---

## 5. ของค้าง

1. **ยังไม่ได้ทดสอบขั้นสุดท้ายของการอนุมัติ** — ทดสอบถึงขั้น 1 อนุมัติแล้วส่งต่อขั้น 2
   ส่วนการอนุมัติขั้นสุดท้าย (ใบเปลี่ยนเป็น "อนุมัติแล้ว" + แจ้งผู้ขอ) และเส้นทาง "ไม่อนุมัติ"
   ยังไม่ได้กดจริงเพราะต้องล็อกอินเป็นผู้อนุมัติขั้นที่ 2 · ตรรกะสองเส้นทางนี้ผ่านการทดสอบ
   ในชั้น `resolveDecision()` แล้ว (ดู §4) · **ยังไม่ได้ยืนยันว่าอีเมล/LINE ออกจริง**
