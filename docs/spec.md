...
> **สถานะเอกสาร:** v1.3 — ผ่านการสัมภาษณ์เก็บ requirements แล้ว 19 ข้อ · Phase 0 เสร็จ · เพิ่ม §16 มาตรฐาน Git & Commit (1 กันยายน 2569) · เพิ่มข้อ 19 ผู้รับผิดชอบหลายคน + auto-assign ตามภาระงาน (4 กันยายน 2569) · เพิ่ม §19 ค้นหาผู้แจ้งจาก Google Workspace Directory (10 กันยายน 2569) · เพิ่ม §20 ตัวกรองหน้า Ticket + admin ตรวจสอบ My Work ของทุกคน (15 กันยายน 2569)
...
20. [ตัวกรองหน้า Ticket + admin ตรวจสอบ My Work ของทุกคน](#20-ticket-filter-admin-mywork)
21. [หัวข้อบริการในบันทึกเวลา + ตัวกรองเริ่มต้น + ยุบเมนูของ admin](#21-worklog-category-menu)
...
| My Work / Time Log | ❌ | ❌ | ✅ | ✅ | ✅ |
| ดู My Work ของผู้อื่น (รวมงานส่วนตัว + เวลาที่ใช้) — อ่านอย่างเดียว · §20 | ❌ | ❌ | ❌ | ❌ | ✅ |
...
<a id="18-department-channel"></a>

## 18. ช่องทาง "ติดต่อจากหน่วยงาน" + ทะเบียนหน่วยงาน (10 กันยายน 2569)

> เลขหัวข้อข้าม 17 ไว้ให้ §17 "ย้ายฐานข้อมูลมาใช้ PostgreSQL บน Docker" ที่ยังอยู่บน branch `feat/postgres-docker`

### 18.1 ที่มา

ช่องทางที่รับแจ้งเดิมมีตัวเลือก *"ติดต่อด้วยตนเอง"* ซึ่งบอกได้แค่ว่าผู้แจ้งเดินมาเอง
แต่ไม่ได้บอกว่ามาจากหน่วยงานไหน จึงแยกรายงานตามหน่วยงานไม่ได้
ผู้ใช้ขอเปลี่ยนเป็น **"ติดต่อจากหน่วยงาน"** พร้อมช่องค้นหาหน่วยงาน และต้องแก้ไขทะเบียนหน่วยงานเองได้

**แหล่งข้อมูล:** `docs/รหัสงานสารบรรณ เรียงจากไฟล์ของฝ่ายบัญชี  .docx` (สรุป ณ 21 สิงหาคม 2569)
— 371 หน่วยงาน รหัส 6 หลักไม่ซ้ำ ลำดับชั้น 3 ระดับ (สำนัก/คณะ 38 · ฝ่าย 161 · งาน 172)

### 18.2 ข้อตกลง

| # | ประเด็น | ข้อสรุป |
|---|---|---|
| 1 | โครงสร้างข้อมูล | เก็บลำดับชั้นด้วย `parentId` + `level` (ไม่ใช่แบนราบ) |
| 2 | การบังคับกรอก | เลือกช่องทางนี้แล้ว **ต้องระบุหน่วยงาน** ทั้งฝั่งฟอร์มและฝั่ง API |
| 3 | สิทธิ์แก้ไขทะเบียน | **manager ขึ้นไป** (จึงอยู่ใต้ `/management` ไม่ใช่ `/admin` ที่บังคับ admin) |
| 4 | ความสะอาดข้อมูล | นำเข้าตามต้นฉบับก่อน (หมายเหตุ/คำสะกดผิดติดมาด้วย) แล้วแก้ในหน้าจอ |

> ⚠️ **ค่าที่เก็บใน DB ยังเป็น `walkin` เหมือนเดิม** — เปลี่ยนเฉพาะป้ายภาษาไทยใน
> `TICKET_CHANNEL_LABEL` เพื่อไม่ต้อง migrate ข้อมูล Ticket เก่า และไม่ต้องแก้ทั้ง Neon
> และ Postgres ใน Docker ให้ตรงกันในช่วงที่ยังไม่ cutover · ป้ายไหลไปทุกที่เองเพราะทุกจุด
> (หน้ารายละเอียด, Excel export, กราฟรายงาน) อ่านจาก map เดียวกัน

### 18.3 สิ่งที่เปลี่ยน

| กลุ่ม | ไฟล์ | สิ่งที่ทำ |
|---|---|---|
| Schema **[M1]** | `prisma/schema.prisma` · `prisma/migrations/20260910100000_department_tree/` | `Department` เพิ่ม `parentId` (self-relation `DepartmentTree`), `level`, index 2 ตัว — additive ล้วน |
| นำเข้าข้อมูล | `scripts/extract-departments.mjs` · `prisma/data/departments.json` · `prisma/data/departments.sql` | อ่าน `.docx` เอง (ตัวอ่าน ZIP ในสคริปต์ ไม่เพิ่ม dependency) รวมชื่อที่ตกบรรทัด 29 แถว แล้วออกเป็น JSON (ตรวจทาน) + SQL (นำเข้าจริง) |
| ป้าย + validation | `lib/ticket-workflow.ts` · `lib/ticket-schema.ts` · `app/api/tickets/route.ts` | เปลี่ยนป้าย `walkin` · เพิ่มค่าคงที่ `DEPARTMENT_CHANNEL` · zod `.refine()` บังคับ `departmentId` · ตรวจว่าหน่วยงานมีจริงและ active |
| ฟอร์มแจ้งปัญหา | `app/(main)/service/tickets/new/NewTicketContent.tsx` | ช่องค้นหาหน่วยงาน (debounce 350ms) แสดงเมื่อเลือกช่องทางนี้ · ล้างค่าเมื่อเปลี่ยนช่องทาง |
| API หน่วยงาน | `app/api/departments/route.ts` · `app/api/departments/[id]/route.ts` | GET เพิ่ม `q`/`all`/`page`/`pageSize` แบบ opt-in · POST/PATCH/DELETE (manager+) · soft-delete เมื่อมีข้อมูลอ้างอยู่ |
| หน้าจัดการ | `app/(main)/management/departments/*` · `lib/department-{schema,service,types}.ts` | ตาราง + ค้นหา + แบ่งหน้า + เพิ่ม/แก้/ย้ายแม่/ปิดใช้งาน/ลบ |
| สิทธิ์ | `lib/screen-access.ts` · `sidebar-data.ts` **[M5]** | กลุ่มใหม่ `ORG_CONFIG` (minRole `manager`) + เมนู "ข้อมูลองค์กร › หน่วยงาน" |

**กติกาที่บังคับในโค้ด**
1. `level` คำนวณจากหน่วยงานแม่ฝั่ง server เสมอ — client ส่งมาไม่ได้
2. ลึกได้ 3 ชั้น (`MAX_DEPARTMENT_LEVEL`) · ย้ายแม่แล้ว `level` ของลูกขยับตาม
3. กันวงวน — ตั้งแม่เป็นตัวเองหรือลูกหลานตัวเองไม่ได้ (`isDescendantOf`)
4. ลบได้จริงเฉพาะเมื่อไม่มี user / ticket / asset / หน่วยงานย่อยอ้างอยู่ ไม่งั้นปิดใช้งานแทน
5. `GET /api/departments` เรียกเปล่าๆ ยังคืน `{ departments }` เหมือนเดิม — หน้าครุภัณฑ์ใช้อยู่

> 🔸 **เรียงตามรหัสแทนชื่อ** — เพราะรหัสงานสารบรรณเรียงตามลำดับชั้นอยู่แล้ว
> dropdown หน้าครุภัณฑ์จึงเปลี่ยนจากเรียงตามชื่อเป็นเรียงตามรหัส (จัดกลุ่มอ่านง่ายกว่าเมื่อมี 372 รายการ)

### 18.4 เหตุผลที่ต้องแสดงรหัสและหน่วยงานแม่ทุกที่

ชื่อหน่วยงาน **ซ้ำกัน 25 กลุ่ม** — `สำนักงานธุรการและประสานงาน` มีถึง **13 แห่ง** ใต้คณะต่างกัน
ทุกจุดที่แสดงหน่วยงาน (ช่องค้นหา, ตาราง, chip ที่เลือกแล้ว) จึงต้องมี **รหัส + เส้นทางหน่วยงานแม่** กำกับ
ไม่งั้นผู้ใช้เลือกผิดแน่นอน

### 18.5 ผลทดสอบ (10 กันยายน 2569)

เกต §16.4 ผ่านครบ: G1 · G2 · G3 · G4 (0 error) · G5 (0 error เฉพาะไฟล์ที่แตะ) · G6 · G7

| # | สิ่งที่ทดสอบ | ผล |
|---|---|---|
| 1 | migration | ✅ `migrate deploy` ผ่าน · SQL มีแต่ `ADD COLUMN` / `CREATE INDEX` / `ADD CONSTRAINT` ไม่มี `DROP`/`TRUNCATE` |
| 2 | นำเข้าข้อมูล | ✅ 371 แถวใหม่ + `OIT` เดิม = **372** · ผูกแม่ครบ 333 แถว · ระดับ 39/161/172 |
| 3 | รันนำเข้าซ้ำ | ✅ **idempotent จริง** — จำนวนแถวเท่าเดิม และ**ไม่ทับชื่อที่แก้ไว้แล้ว** (ต่างจากบั๊ก seed เดิม) |
| 4 | ลำดับชั้น | ✅ `010103 งานสารบรรณ` → `010100 ฝ่ายบริหารทั่วไป` → `010000 สำนักอธิการบดี` |
| 5 | ป้ายช่องทาง | ✅ dropdown ขึ้น "ติดต่อจากหน่วยงาน" · หน้ารายละเอียด Ticket ขึ้นป้ายเดียวกัน |
| 6 | บังคับเลือกหน่วยงาน | ✅ กดส่งโดยไม่เลือก → ถูกบล็อกพร้อมข้อความไทย |
| 7 | ค้นหาชื่อซ้ำ | ✅ ค้น "ธุรการและประสานงาน" ได้หลายรายการ แยกออกด้วยรหัส + หน่วยงานแม่ |
| 8 | สร้าง Ticket จริง | ✅ `TK-256909-00012` · `channel = walkin` · `departmentId` = `010103 งานสารบรรณ` |
| 9 | แก้ไขในหน้าจอ | ✅ แก้ `010200` จาก "ฝ่ายอาคารถานที่" → "ฝ่ายอาคารสถานที่" · ลูกยังผูกอยู่ · level ไม่เพี้ยน |
| 10 | ไม่พังของเดิม | ✅ หน้าครุภัณฑ์โหลด dropdown หน่วยงานได้ครบ 372 รายการ |
| 11 | สิทธิ์ | ✅ ไม่ login → API 401 ทุกเส้น · หน้า `/management/departments` redirect ไป signin |

**ยังไม่ได้ทดสอบ:** สิทธิ์ระดับ role (manager เข้าได้ / agent ถูกเด้ง) — ต้องใช้บัญชีคนจริงจึงยังไม่ทำ

### 18.6 ของค้าง

1. **ข้อมูลจะหายตอน cutover** — §17 ของ branch `feat/postgres-docker` ระบุว่า cutover ทำ
   `DROP SCHEMA public CASCADE` แล้ว restore จาก Neon ทับ · migration + ข้อมูล 371 หน่วยงานที่ลง
   Postgres Docker ไว้ตอนนี้**จะถูกล้างทิ้ง** ต้อง `migrate deploy` + นำเข้าซ้ำหลัง cutover
   (หรือ apply ลง Neon ด้วย เพราะคอลัมน์เป็น nullable/มี default แอปเก่าไม่พัง)
...
<a id="20-ticket-filter-admin-mywork"></a>

## 20. ตัวกรองหน้า Ticket + admin ตรวจสอบ My Work ของทุกคน (15 กันยายน 2569)

Branch: `feat/itsm-ticket-filter-admin-mywork`

### 20.1 ความต้องการ

1. หน้ารายการ Ticket
   - แสดงตัวกรอง "แจ้งใหม่" แทน "ทั้งหมด" เป็นค่าเริ่มต้น และย้าย "ทั้งหมด" ไปไว้ท้ายสุด
   - มีวิธีดูรายการที่ยังไม่เสร็จ
   - เรียง Ticket ล่าสุดไว้ด้านบน
2. admin เข้าไปตรวจสอบ My Work ของทุกคนได้ รวมถึงเวลาที่ใช้ไปในแต่ละรายการ

### 20.2 ข้อตกลง

| # | เรื่อง | ข้อตกลง |
|---|---|---|
| 1 | ตัวกรองสถานะ | ลำดับ: **แจ้งใหม่** (ค่าเริ่มต้น) · ยังไม่เสร็จ · มอบหมายแล้ว · กำลังดำเนินการ · แก้ไขแล้ว · ปิดงาน · **ทั้งหมด** · "ล้างตัวกรอง" กลับไปที่แจ้งใหม่ |
| 2 | ยังไม่เสร็จ | ปุ่มเดียวส่ง `status=new,assigned,in_progress` (`OPEN_STATUSES`) — API รับหลายสถานะคั่นด้วย comma อยู่แล้ว ไม่ต้องแก้ API |
| 3 | การเรียง | ค่าเริ่มต้นเปลี่ยนจาก `queue` เป็น `newest` (`createdAt DESC`) · ยังเลือก "เรียงตามคิวงาน" ได้ |
| 4 | การ์ดสรุปใบแรก | เปลี่ยน label "ทั้งหมด" → "ตรงตามตัวกรอง" เพราะตัวเลขมาจากชุดที่กรองแล้ว |
| 5 | ใครดู My Work ของผู้อื่นได้ | **admin เท่านั้น** (manager ยังดูได้แค่รายงานภาระงานเหมือนเดิม) |
| 6 | ขอบเขตที่เห็น | ทุกแท็บ: Ticket · Task · **งานส่วนตัว** · บันทึกเวลา — ⚠️ **กลับกฎเดิม** ที่ว่างานส่วนตัวเจ้าของเห็นคนเดียวแม้แต่ admin (ผู้ใช้ยืนยัน 15 ก.ย. 2569) |
| 7 | สิทธิ์แก้ไข | **อ่านอย่างเดียว** — route แก้/ลบ todo และ worklog ยังตรวจว่าเป็นเจ้าของ (`ownerId`/`userId === me`) เหมือนเดิม |
| 8 | เวลาที่ใช้ | `loggedMinutes` = นาทีรวมจาก `WorkLog` ที่**เจ้าของ My Work** ลงไว้กับงานนั้น · แสดงเป็นคอลัมน์ในรายการงาน (เจ้าตัวก็เห็น) และในงานส่วนตัว |

### 20.3 API

ทุกเส้นตรวจสิทธิ์ที่ server: ส่ง id ของคนอื่นมาโดยไม่ใช่ admin → `403`

| Route | พารามิเตอร์ใหม่ | หมายเหตุ |
|---|---|---|
| `GET /api/my-work` | `userId` | เติม `loggedMinutes` ด้วย `attachLoggedMinutes()` หลังตัดรายการแล้ว (groupBy ครั้งเดียว) · แดชบอร์ดที่ใช้ `loadWorkItems()` ไม่เสีย query เพิ่ม |
| `GET /api/todos` | `ownerId` | เพิ่ม `loggedMinutes` ต่องาน · POST ยังสร้างในนามตัวเองเท่านั้น |
| `GET /api/worklogs/summary` | `userId` (คู่กับ `scope=own`) | |
| `GET /api/worklogs` | — | มี `userId` อยู่แล้ว (manager ขึ้นไป — F3.8) |

### 20.4 หน้าจอ

- `service/tickets` — ตามข้อตกลง 1–4
- `service/my-work` — admin เห็น dropdown **"ดูงานของ"** (รายชื่อจาก `/api/directory?scope=agents`) · เลือกแล้ว URL เป็น `?userId=` รีเฟรช/แชร์ลิงก์ได้ · หัวข้อเปลี่ยนเป็น "งานของ {ชื่อ}" + แถบ "โหมดอ่านอย่างเดียว" + ปุ่มกลับไปงานของฉัน · ซ่อนปุ่มเพิ่ม/แก้/ลบ และปิด checkbox ติ๊กเสร็จ
- `management/reports/workload` — admin กดชื่อเจ้าหน้าที่เพื่อเข้าไปตรวจสอบ My Work ของคนนั้น

<a id="21-worklog-category-menu"></a>

## 21. หัวข้อบริการในบันทึกเวลา + ตัวกรองเริ่มต้น + ยุบเมนูของ admin (16 กันยายน 2569)

Branch: `feat/itsm-worklog-category-menu`

### 21.1 ความต้องการ

1. เพิ่มช่องเลือก **"หัวข้อบริการ"** จาก Service Catalog ในฟอร์ม "บันทึกเวลาทำงาน" (ตามภาพ `docs/ITSM_sample_v1.png` — กรอบแดงอยู่ระหว่างแถว "ประเภทงาน/ผูกกับงาน" กับช่อง "สิ่งที่ทำ")
2. หน้า "Ticket ทั้งหมด" ให้ตัวกรองเริ่มต้นเป็น **"ยังไม่เสร็จ"** แทน "แจ้งใหม่"
3. ยุบหัวข้อเมนูที่ admin ไม่ค่อยได้ใช้ไว้ตั้งแต่เปิดหน้า: ฐานความรู้ · AI & ข้อมูล · งานธุรการศูนย์ · ลูกค้าสัมพันธ์ · ข้อมูลองค์กร · ตั้งค่าบริการ · ผู้ดูแลระบบ

### 21.2 ข้อตกลง

| # | เรื่อง | ข้อตกลง |
|---|---|---|
| 1 | แหล่งข้อมูลหัวข้อบริการ | `ServiceCategory` (Service Catalog เดิม) — ไม่สร้างตารางใหม่ · `GET /api/categories` เอาเฉพาะหมวดที่ `active` · แสดงเป็น optgroup "หมวดหลัก → หมวดย่อย" เหมือนหน้าแจ้งปัญหา |
| 2 | บังคับกรอกเมื่อไร | **บังคับเฉพาะประเภท "งานประจำ" (`refType = other`)** เพราะไม่มีใบสั่งงานให้อ้างอิงว่าเวลาหมดไปกับบริการเรื่องใด · ประเภทอื่นเลือกหรือเว้นว่างก็ได้ (ผู้ใช้ยืนยัน 16 ก.ย. 2569) |
| 3 | ข้อมูลเดิม | คอลัมน์ `work_log.categoryId` เป็น NULL ได้ — บันทึกเวลาที่มีอยู่แล้วไม่ต้องย้อนกรอก · ตอนแก้บันทึกเก่าให้เป็น "งานประจำ" จึงจะถูกบังคับ |
| 4 | ลบหมวดหมู่ | FK เป็น `ON DELETE SET NULL` — ลบหมวดแล้วบันทึกเวลายังอยู่ เพราะเวลาที่ลงไว้เป็นข้อเท็จจริงของคนทำงาน |
| 5 | ปิด Ticket แล้วลงเวลาอัตโนมัติ | `PATCH /api/tickets/{id}/status` ที่สร้าง WorkLog ตอน `resolved` เติม `categoryId` จากหมวดของ Ticket ใบนั้นให้เอง |
| 6 | ตัวกรองหน้า Ticket | `DEFAULT_STATUS` เปลี่ยนจาก `new` เป็น `open` (= `OPEN_STATUSES`) · ลำดับปุ่มและปุ่ม "ล้างตัวกรอง" ยังเหมือน §20 แต่กลับไปที่ "ยังไม่เสร็จ" |
| 7 | การยุบเมนู | เป็นแค่ **ค่าเริ่มต้น** — กดกางเองได้ และสถานะจะอยู่จนกว่าจะรีเฟรช · role อื่นยังกางทุกหัวข้อเหมือนเดิม |

### 21.3 Data Model

```prisma
model WorkLog {
  categoryId String?          // หัวข้อบริการ (NULL ได้ — บันทึกเดิมไม่มี)
  category   ServiceCategory? @relation(fields: [categoryId], references: [id])
  @@index([categoryId])
}
```

Migration: `prisma/migrations/20260916100000_worklog_category/` — `ADD COLUMN` + index + FK `ON DELETE SET NULL` (เป็นการเพิ่มอย่างเดียว ไม่แตะข้อมูลเดิม)

### 21.4 API

| Route | ที่เปลี่ยน |
|---|---|
| `POST /api/worklogs` | รับ `categoryId` (nullable) · zod บังคับเมื่อ `refType = other` |
| `PATCH /api/worklogs/{id}` | รับ `categoryId` · รวมกับค่าเดิมก่อนตรวจ — แก้ประเภทเป็น "งานประจำ" โดยไม่มีหัวข้อ → `400` |
| ทั้งสองเส้น | `validateWorkLogRef()` ตรวจกับ DB ว่าหมวดมีจริงและยัง `active` — กัน id มั่วจากภายนอก |
| response ของทุกเส้น | เพิ่ม `categoryId` และ `categoryName` ("หมวดหลัก › หมวดย่อย") |

### 21.5 หน้าจอ

- `service/my-work` แท็บ **บันทึกเวลา** — ช่อง "หัวข้อบริการ" อยู่ใต้แถว "ประเภทงาน/ผูกกับงาน" · ติดดอกจันเมื่อเลือก "งานประจำ" · รายการบันทึกเวลาแสดงหัวข้อบริการต่อท้ายประเภทงาน
- `service/tickets` — เปิดหน้ามาที่ "ยังไม่เสร็จ"
- Sidebar — `NavSectionType.collapsedForRoles` บอกว่า role ไหนให้ยุบหัวข้อนี้ไว้ก่อน · `Sidebar` ใส่ค่านี้ไว้ใน `key` ของ `NavSection` เพื่อให้ค่าเริ่มต้นถูกต้องเมื่อ session โหลดเสร็จแล้ว role เปลี่ยน
