...
1. ~~**บทความ `agent_only` ควรเข้า RAG ไหม?**~~ — **ตอบแล้ว: ไม่ควรให้ตอบผู้ใช้ทั่วไป**
   แก้ใน branch `fix/kb-agent-only-visibility` โดยกรองที่ชั้น vector search (ดู §8)
...
## 8. งานตามเก็บหลังปิดเฟส

### กันบทความ `agent_only` ไม่ให้ถูกเอาไปตอบผู้ใช้ทั่วไป

branch `fix/kb-agent-only-visibility` · commit `fix(kb): กันแชตบอทเอาบทความ agent_only ไปตอบผู้ใช้ทั่วไป`

บทความ `agent_only` ถูก sync เข้า pgvector เหมือนบทความทั่วไป `searchDocuments()` จึงดึงมาตอบ
ใครก็ได้ที่ถามแชตบอท ทั้งที่หน้าเว็บกันไว้แล้ว — แก้โดยกรองตั้งแต่ใน SQL ของ vector search
ย้อนจาก `metadata.source` (`kb:<id>`) กลับไปหาบทความต้นทาง แล้วรับเฉพาะ
`status = 'published'` และ `visibility = 'all'`

**ค่าตั้งต้นของ `searchDocuments()` คือ "กรอง"** — ผู้เรียกต้องส่ง `includeAgentOnly: true`
เข้ามาเองถึงจะเห็นบทความเฉพาะเจ้าหน้าที่ ออกแบบแบบนี้เพื่อให้จุดเรียกที่ลืมส่ง option
ได้พฤติกรรมที่ปลอดภัยเสมอ ครอบคลุม `api/search` และ webhook ของ LINE ไปด้วยโดยไม่ต้องแก้

| ไฟล์ที่แก้ | อยู่ในตาราง M? |
|---|---|
| `lib/vector-search.ts` | นอกตาราง — ต่อจากงานเฟส 6 ที่ขออนุมัติแล้ว |
| `lib/rag-service.ts` | **M10** |
| `app/api/chat/route.ts` | นอกตาราง (งานแก้บั๊กนอกเฟส) |
| `app/api/chat/stream/route.ts` | นอกตาราง (งานแก้บั๊กนอกเฟส) |

**ทดสอบกับฐานข้อมูลจริงครบ 6 กรณี** (สร้างบทความจำลองใน transaction แล้ว `ROLLBACK`
ไม่แตะข้อมูลจริง): บทความเปิดทุกคน ✓ เห็น · `agent_only` ✓ ไม่เห็น · ฉบับร่าง ✓ ไม่เห็น ·
vector กำพร้า ✓ ไม่เห็น · เอกสารที่อัปโหลดเอง ✓ เห็น · `metadata` ไม่มี `source` ✓ เห็น

> **สิ่งที่เจอระหว่างแก้และยังไม่ได้ทำ:** `app/api/search/route.ts` **ไม่มีการตรวจ session เลย**
> เปิดให้ใครก็ได้ยิง vector search ดิบ ตอนนี้เนื้อหา KB ปลอดภัยแล้วเพราะค่าตั้งต้นกรองให้
> แต่เอกสารที่อัปโหลดเข้าระบบยังหลุดได้อยู่ — ควรเพิ่ม `requireAuth()` ในเฟสถัดไป
