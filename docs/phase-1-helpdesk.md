...
## 0. ⏸️ จุดที่หยุดพัก — อ่านข้อนี้ก่อนเมื่อกลับมาทำต่อ

**หยุดเมื่อ:** 1 กันยายน 2569 · โค้ดเสร็จและ merge เข้า `main` แล้ว **แต่ยังไม่ได้ push**

### ต้องทำทันทีเมื่อกลับมา

| # | สิ่งที่ต้องทำ | คำสั่ง / หมายเหตุ |
|---|---|---|
| 1 | **push `main` ขึ้น GitHub** — ค้างอยู่ **9 commit** (รวม merge commit ของเฟส) เพราะ `git push` ถูกตัวกรองสิทธิ์ของ Claude Code บล็อก ต้องรันเอง | `git push origin main` → ปลายทาง `rattanawong-it/AI-Native-App` |
| 2 | ตรวจว่า push แล้วไม่มีอะไรค้าง | `git log --oneline origin/main..main` ต้องว่าง |
| 3 | ลบ branch เก่าเมื่อยืนยันว่าไม่มีปัญหา (§16.1) | `git branch -d feat/itsm-phase-0` — เก็บ `feat/itsm-phase-1` ไว้จนเปิด Phase 2 |

### สถานะ ณ จุดหยุด

| รายการ | สถานะ |
|---|---|
| โค้ด Phase 1 | ✅ เสร็จ 19/21 · ทดสอบ end-to-end ผ่านหน้าเว็บจริงแล้ว (§5.1) |
| เกต G1–G7 | ✅ ผ่านครบ — tsc 0 error · eslint 0 error · build ผ่าน |
| merge เข้า `main` | ✅ `022bec7` แบบ `--no-ff` |
| push ขึ้น GitHub | ⛔ **ยังไม่ได้ทำ** (ดูตารางบน) |
| ข้อมูลทดสอบใน Neon | ✅ ลบหมดแล้ว — ข้อมูลเดิมทุกตารางเท่าเดิม |
| working tree | ✅ สะอาด |
| `pnpm dev` | รันค้างไว้ตอนทดสอบ — ปิดได้เลย ไม่กระทบอะไร |

### ต้องตัดสินใจก่อนเปิด Phase 2

| # | เรื่อง | ทางเลือก |
|---|---|---|
| 1 | **F1.7 จะเก็บไฟล์แนบที่ไหน** | local disk ตาม `UPLOAD_DIR` (ต้อง mount volume บน Docker ไม่งั้นไฟล์หายตอน redeploy) · เก็บลง DB เป็น bytea (ต้องแก้ schema) · object storage |
| 2 | **`stash@{0}` จะเอาอย่างไร** | เก็บการแก้ที่ถอด guard ของ `LINE_CHANNEL_SECRET` และการตรวจ signature ออก — **แนะนำให้ทิ้ง** (`git stash drop`) เพราะเป็นการถอยด้านความปลอดภัย · ถ้าจะเก็บไว้ดูก่อน ปล่อยไว้ได้ ไม่กระทบงาน |
| 3 | **เมนู "คิวงานทีม" จะใส่ใน sidebar ไหม** | ถ้าใส่ ต้องขออนุมัติแก้ `sidebar-data.ts` (M5) · ตอนนี้เข้าจากปุ่มบนหน้า Ticket ทั้งหมดได้อยู่แล้ว |
| 4 | **รายชื่อเจ้าหน้าที่จริง** | ยังไม่มี → `defaultAssigneeId` ว่างทุกหมวด auto-assign จึงได้แค่ทีม (spec §14 ข้อ 6) |

### Phase 2 คืออะไร

**SLA Engine + Business Hours + SLA Report (spec §8 ④ · F4.1–F4.11)** — พึ่งพา Phase 1
งานหลัก: หน้า `admin/sla` (CRUD `SlaPolicy`) · หน้า `admin/calendar` (CRUD `BusinessHour` + `Holiday`) · รายงาน SLA Compliance · รายการ Ticket ที่ Breach
ตัวคำนวณ (`lib/business-hours.ts`) และการตั้งธง breach ทำไว้แล้วใน Phase 0–1 เหลือทำหน้าตั้งค่ากับรายงาน
...
0. ⛔ **ยังไม่ได้ push `main` ขึ้น GitHub** — ค้าง 9 commit · `git push` ถูกตัวกรองสิทธิ์ของ Claude Code บล็อก ต้องรัน `git push origin main` เอง (ดู §0)
...
| `eeeb74a` | `docs` เพิ่มผลทดสอบหน้าเว็บจริงและบั๊กที่แก้ |
| `022bec7` | **Merge** `feat/itsm-phase-1` เข้า `main` แบบ `--no-ff` |
| *(commit นี้)* | `docs` บันทึกจุดหยุดพักและของค้างก่อนพัก |

*(`cd8596d` docs §16 มาตรฐาน Git ทำไว้ก่อนเปิดเฟส และไป `main` พร้อมกันตอน merge)*

> **หมายเหตุลำดับ commit:** commit นี้ทำบน `feat/itsm-phase-1` หลัง merge ไปแล้ว
> จึงต้อง merge เข้า `main` อีกรอบ — ทำแบบนี้เพื่อไม่ commit ลง `main` ตรงๆ ตาม §16.1
