...
- [ ] **F0.1** ขยาย `prisma/schema.prisma` เพิ่ม ~24 models ใหม่ **[M1]**
- [ ] **F0.2** เพิ่ม field ใน model `User`: `departmentId`, `position`, `phone`, `employeeCode`, `lineUserId`, `isAgent` **[M2]**
- [ ] **F0.3** สร้าง migration + ทดสอบ `pnpm prisma migrate dev`
- [ ] **F0.4** ขยาย `lib/permissions.ts` — statement ใหม่ + role `student`, `agent` **[M3]**
- [ ] **F0.5** ลงทะเบียน role ใหม่ใน `lib/auth.ts` **[M4]**
- [ ] **F0.6** สร้าง `lib/rbac.ts` — helper `requireRole()`, `canAccessTicket()` ใช้ร่วมทุก API
- [ ] **F0.7** เพิ่ม shadcn components: `table`, `dialog`, `select`, `badge`, `tabs`, `textarea`, `dropdown-menu`, `avatar`, `separator`, `sheet`, `popover`, `calendar`, `checkbox`, `switch`, `sonner`, `skeleton`, `alert-dialog`, `progress`, `tooltip` **[M12]**
- [ ] **F0.8** อัปเดต `sidebar-data.ts` เพิ่ม section `Service` + item ใหม่ + `allowedRoles` **[M5]**
- [ ] **F0.9** สร้าง `lib/business-hours.ts` — คำนวณ due date ตามเวลาทำการ + วันหยุด
- [ ] **F0.10** สร้าง `lib/priority.ts` — Impact × Urgency Matrix
- [ ] **F0.11** สร้าง `lib/running-number.ts` — generate `TK-YYYYMM-00001`, `RQ-YYYYMM-0001`
- [ ] **F0.12** ขยาย `prisma/seed.ts` — Service Catalog, SLA Policy, Business Hours, วันหยุดราชการไทย, ทีมตัวอย่าง
- [ ] **F0.13** เพิ่ม dependencies ใน `package.json` **[M11]**
